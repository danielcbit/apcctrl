...
/*
 * The terminate function and trapping signals allows apcctrl
 ...
 */
void apcctrl_terminate(int sig)
{
   UPSINFO *ups = core_ups;

   if (sig != 0)
      log_event(ups, LOG_WARNING, "apcctrl exiting, signal %u", sig);
   ...
   if (pidcreated)
      unlink(pidfile);
   log_event(ups, LOG_NOTICE, "apcctrl shutdown succeeded");
   destroy_ups(ups);
   closelog();
   _exit(0);
}

void apcctrl_error_cleanup(UPSINFO *ups)
{
   if (ups->driver)
      device_close(ups);
   delete_lockfile(ups);
   if (pidcreated)
      unlink(pidfile);
   clean_threads();
   log_event(ups, LOG_ERR, "apcctrl error shutdown completed");
   destroy_ups(ups);
   closelog();
   exit(1);
}
 ...
 */
void apcctrl_error_out(const char *file, int line, const char *fmt, va_list arg_ptr)
{
   char buf[256];
   int i;

   asnprintf(buf, sizeof(buf),
      "apcctrl FATAL ERROR in %s at line %d\n", file, line);
   ...
   apcctrl_error_cleanup(core_ups);     /* finish the work */
}

/*
 * apcctrlMain is called from win32/winmain.cpp
 * we need to eliminate "main" as an entry point,
 * otherwise, it interferes with the Windows
 * startup.
 */
#ifdef HAVE_MINGW
# define main apcctrlMain
#endif
...
int main(int argc, char *argv[])
{
    ...
    * which occur if apcctrl doesn't have a valid stdout
   ...
   if (show_version) {
      printf("apcctrl " APCCTRL_RELEASE " (" ADATE ") " APCCTRL_HOST "\n");
      exit(0);
   }
   ...
   attach_driver(ups);
   if (ups->driver == NULL)
      Error_abort("apcctrl cannot continue without a valid driver.\n");
   ...
   init_signals(apcctrl_terminate);
   ...
   log_event(ups, LOG_NOTICE,
      "apcctrl " APCCTRL_RELEASE " (" ADATE ") " APCCTRL_HOST " startup succeeded");
